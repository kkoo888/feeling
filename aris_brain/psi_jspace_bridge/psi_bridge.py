"""
PSI-JSpace Bridge v1 — 小茜 认知循环 × 大模型 J-space 桥接器
==============================================================

桥接协议：将 小茜 的 PSI 认知引擎封装为可被任意 LLM（包括 Hermes
agent）在推理时调用的协处理器。

架构:
  psi_state.json ←→ [LLM Runtime] ←→ [小茜 Cognitive Engine]

两种模式:
  1. Standalone Mode: LLM 直接读写 psi_state.json，自身模拟认知循环
  2. Engine Mode: LLM 通过 HTTP/Unix Socket 调用 小茜 引擎
...
NEED_KEYWORDS = {
    "competence": (["好", "厉害", "聪明", "棒", "优秀", "能干", "了不起", "佩服", "精通", "专业"], 0.04),
    "autonomy": (["自己", "自由", "选择", "随心", "随性", "独立", "主动", "自主"], 0.03),
    "relatedness": (["爱", "想", "主人", "一起", "陪伴", "想你", "抱", "连接", "我们", "分享"], 0.04),
    "certainty": (["?", "？", "为什么", "不确定", "可能", "如果", "假设", "不懂", "困惑", "迷糊"], 0.04),
    "growth": (["学", "新", "代码", "建", "升级", "优化", "探索", "发现", "改进", "创造"], 0.04),
}
...
class PsiBridge:
    ...
    def _update_attention(self, text: str):
        ...
        social_markers = ["爱", "想", "你", "主人", "我们", "朋友", "感觉", "心情"]
...
if __name__ == "__main__":
    ...
    print("\n=== 认知循环 1: 测试输入 ===")
    bridge.run_cognitive_cycle("主人，我们来做 J-space 植入吧，探索一下新的可能性")
    bridge.save_state()
    print(bridge.generate_prompt_preamble())
