...
try:
    from laap_brain.config import BRAIN_DIR as LAAP_BRAIN
except ImportError:
    LAAP_BRAIN = Path(os.environ.get("LAAP_BRAIN_ROOT",
...
class PsiSemioticsBridge:
    ...
    def ensure_loaded(self):
        ...
        # 尝试加载持久化符号库
        if self.engine:
            save_path = str(LAAP_BRAIN / "state" / "psi_semiotics.json")
    ...
    def save(self):
        """保存符号库状态"""
        if self.engine:
            self.engine.save(str(LAAP_BRAIN / "state" / "psi_semiotics.json"))
    
    def load(self):
        """加载符号库状态"""
        if self.engine:
            self.engine.load(str(LAAP_BRAIN / "state" / "psi_semiotics.json"))
